"""
PTC10 resume plan for user Zhang — restart from 240 °C.

Previous scan was stopped at 240 °C.  This plan picks up from there:
  1. Heat to 240 °C at 5 °C/min (no data — catching up to stop point).
  2. Continue ramp to 550 °C at 5 °C/min, collecting data during ramp.
...
Reload:
    %run -im usaxs.user.PTC10_zhang_resume240
...
Usage:
    RE(PTC10_zhang_resume240("MySample"))
"""
...
HEAT_RATE_C_PER_MIN = 5.0   # °C/min
TEMP_RESUME = 240.0          # °C — resume point (where previous scan stopped)
...
def PTC10_zhang_resume240(sample_name, md={}):
    """
    PTC10 resume plan for user Zhang, restarting from 240 °C.

    Fixed position: sx=0, sy=0, thickness=1.3 mm.

    Sequence:
        1. Startup (before_command_list).
        2. Heat to 240 °C at 5 °C/min, wait for arrival (no data — resume point).
        3. Ramp to 550 °C at 5 °C/min, collecting data during ramp.
    ...
    Examples
    --------
    RE(PTC10_zhang_resume240("LiMnO"))
    """
    ...
    logger.info(
        "Starting PTC10_zhang_resume240 | sample=%s | debug=%s",
        sample_name,
        isDebugMode,
    )
    ...
    appendToMdFile(
        f"## PTC10_zhang_resume240: {sample_name}\n"
        f"- Position: sx={POS_X}, sy={POS_Y}, thickness={THICKNESS} mm\n"
        f"- Resuming from {TEMP_RESUME} °C (previous scan stopped here)\n"
        f"- Heat to {TEMP_TARGET} °C at {HEAT_RATE_C_PER_MIN} °C/min\n"
        f"- Hold for {HOLD_MINUTES} min then cool to {COOL_TO} °C"
    )

    # --- Block 2: Heat to 240 °C (resume point — no data collection) --------
    ptc10.tolerance.put(2.0)
    logger.info("Block 2: heating to %.0f °C (resume point, no data)", TEMP_RESUME)
    appendToMdFile(f"Heating to {TEMP_RESUME} °C (resume point — no data collection)")
    yield from bps.mv(ptc10.ramp, HEAT_RATE_C_PER_MIN / 60.0)  # °C/min → °C/s
    yield from bps.mv(ptc10.temperature.setpoint, TEMP_RESUME)
    yield from setheaterOn()

    while not ptc10.temperature.inposition:
        logger.info("Waiting for %.0f °C | PTC10=%.1f °C", TEMP_RESUME, ptc10.position)
        yield from bps.sleep(5)

    logger.info("At %.0f °C — resuming ramp to %.0f °C", TEMP_RESUME, TEMP_TARGET)
    appendToMdFile(f"Reached {TEMP_RESUME} °C — resuming ramp to {TEMP_TARGET} °C")
    ...
    appendToMdFile(f"PTC10_zhang_resume240 complete: {sample_name}")
