...
def myLinkamPlan_template(pos_X, pos_Y, thickness, scan_title, temp1, rate1, delay1min, temp2, rate2, md={}):
    ...
    checkpoint = time.time() + delay1min*60             # calculate time to end ``delay1min`` hold period
...
def Fan174Plan(pos_X, pos_Y, thickness, scan_title, temp1, rate1, delay1min, temp2, rate2, delay2min, md={}):
    ...
    4. Cool down to 30C, collecting WAXS data
...
def mooreLinkamPlan(pos_X, pos_Y, thickness, scan_title, temp1, temp2, delay2min, md={}):
    """
    0. use linkam_tc1
    1. collect 40C (~RT) USAXS/SAXS/WAXS
    ...
    # Collect data at 40C as Room temperature data.
    yield from change_rate_and_temperature(60,230,wait=True)        # rate for next ramp, default 150C/min,sets the temp of to 40C, waits until we get there (no data collection)
    t0 = time.time()                                     # set this moment as the start time of data collection.
    yield from collectAllThree(isDebugMode)              # collect the data at RT
    ...
    #Heating cycle 1 - ramp up and hold
    logger.info(f"Ramping temperature to {temp1} C")        # for the log file
    yield from change_rate_and_temperature(60,temp1,wait=True)      # set rate & temp this cycle, wait=True waits until we get there (no data collection)
    #yield from change_rate_and_temperature(rate1,temp1,wait=False)     # set rate & temp this cycle, wait=False contniues for data collection
    #   do we want to reset the time here again? 
    t0 = time.time()                                        # set this moment as the start time of data collection.
    # by now are at temp1 and should hold for delay1min:
    checkpoint = time.time() + 3*60             # calculate time to end ``delay1min`` hold period
    logger.info(f"Reached temperature, now collecting data for 5 minutes")
    #this collects data for delay1minm  
    while time.time() < checkpoint:                         # collects USAXS/SAXS/WAXS data while holding at temp1
        yield from collectAllThree(isDebugMode)
    ...
    # set linkam conditions
    yield from change_rate_and_temperature(60,temp2,wait=True)     # set rate & temp this cycle, wait=True waits until we get there (no data collection)
    ...
    t0 = time.time()                                        # set this moment as the start time of data collection.
    # by now are at temp1 and should hold for delay1min:
    checkpoint = time.time() + delay2min*60             # calculate time to end ``delay1min`` hold period
    #this collects data for delay2min  
    while time.time() < checkpoint:                         # collects USAXS/SAXS/WAXS data while holding at temp1
        yield from collectAllThree(isDebugMode)

    yield from change_rate_and_temperature(100,25,wait=False)     # set rate & temp this cycle, wait=False contniues for data collection
